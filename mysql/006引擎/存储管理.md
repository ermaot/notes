...
XDES_STATE | 4|区的状态,有效值为:<p>XDES_FREE该区在FSP_FREE链表中<p>XDES_FREE_FRAG该区是碎片区,在FSP_FREE_FRAG链表中<p>XDES_FULL_FRAG该区是碎片区,在FSP_FULL_FRAG链表中,区中的页都已经被使用<p>XDES_SEG区隶属于某个段
XDES_BITMAP | 16|区中64个页是否被使用的状态,每个页用2位表示(实际仅使用一位信息),定义如下:<p>XDES_FREE_BIT页是否已经被使用<p>XDES_CLEAN_BIT未被使用

- 一个页可以容纳多个区描述符（最多256个）
- 每个区描述符的偏移量都是16384（256*64）的倍数
- 区描述符的起始位置在150（38 字节文件头+ 112space header）字节处
- 若一个区的页含有区描述符，则成为碎片区，不能分配给段
![区描述符偏移量](142660DF096E493A888497DEAB937D1E)
...
```
- 表空间、段、区、页关系
![表空间、段、区、页关系](7EE35893A1E8477DB164C9D41C23E66D)
## 数据结构
#### 概述
#### fil_system_struct
#### fil_space_struct
